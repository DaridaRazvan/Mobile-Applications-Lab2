...
class Item {
  constructor({ id, description, price, orderDate, negotiable, date, version }) {
    this.id = id;
...
const items = [];
for (let i = 0; i < 5; i++) {
  items.push(new Item({ id: `${i}`, description: `item desc: ${i}`, price: `${i}` ,orderDate: new Date(Date.now() + i),negotiable: true,date: new Date(Date.now() + i), version: 1 }));
}
...
const createItem = async (ctx) => {
  ...
  if (!item.description) { // validation
    ctx.response.body = { issue: [{ error: 'Description is missing' }] };
    ctx.response.status = 400; //  BAD REQUEST
    return;
  }
  if(!item.price) {
    ctx.response.body = {issue: [{error: 'Price is missing'}]};
    ctx.response.status = 400; //  BAD REQUEST
    return;
  }
  if(!item.orderDate){
    ctx.response.body = {issue: [{error: 'Date is missing'}]};
    ctx.response.status = 400; //  BAD REQUEST
    return;
  }
...
setInterval(() => {
  ...
  const item = new Item({ id: lastId, description: `item desc: ${lastId}`, price: `${lastId}` ,orderDate: new Date(Date.now()),negotiable: true,date: lastUpdated, version: 1 });
  items.push(item);
  console.log(`
   ${item.text}`);
  broadcast({ event: 'created', payload: { item } });
